			...
			<?} else {?>
				<?foreach($images as $image):?>
					<span class="thumb_wrapper" title="insert '<?=htmlspecialchars($image['description']);?>'">
						<span class="thumb" onclick="TinyCIMMImage.loadResizer('<?=$image['id'].$image['extension'];?>', event);" style="background:url(/assetmanager/image/get/<?=$image['id'];?>/92/92) no-repeat center center;">
