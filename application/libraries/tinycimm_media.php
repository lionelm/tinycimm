...
class TinyCIMM_media extends TinyCIMM {
	...
	**/
	public function get_browser($folder=0, $offset=0, $search='') {
		...
		$pagination_config['base_url'] = base_url($ci->config->item('tinycimm_controller').'media/get_browser/'.$folder);
		...
		$ci->load->view($this->view_path.'media_'.$ci->session->userdata('cimm_view').'_list', $data);
	}
